...
| `S-GEO-001` | Geometric | Raw-path cost-induced directed extended metric | F3, F5 |
| `S-INF-001` | Informational | T-blind admissible history-quotient lattice | F4, F5 |
| `S-DYN-001` | Dynamical | Congruence of action-availability equivalence under action dynamics | F1, F2, F3, F5 |
...
**Non-tautology argument:** The primary claim is an ordinary mathematical consequence of literal finite-sequence concatenation on the common defined domain of the partial operation. Its positive validation is therefore a formal baseline and cannot reject `H_0`.

### Scientific claim

Finite executable paths generated by the declared operational relation admit a canonical typed partial composition by literal path concatenation, with associativity wherever both bracketings are defined.
...
For all typed triples `\pi_1,\pi_2,\pi_3`, whenever both bracketings
...
are defined under the frozen partial-operation semantics, they are equal as literal paths:
...
No claim is made that admissibility of the full literal concatenation forces either intermediate concatenation to be `\kappa`-admissible. No identity structure is claimed unless empty paths are independently part of `\kappa`.
...
`REFUTED_IF:` a valid scoped object has a typed triple for which both bracketings are defined but unequal as literal paths.
...
## B5. `S-GEO-001` — Raw-path cost-induced directed extended metric

**Layer:** Geometric  
**Epistemic class:** `FORMAL_BASELINE`  
**Non-tautology argument:** Under the semantic domain's nonnegative additive raw-path cost assumptions, zero self-distance and the directed triangle inequality are shortest-path consequences of the declared construction. The result is therefore a conditional induced geometry baseline rather than positive evidence that geometry is forced.

### Scientific claim

When the operational accounting convention supplies nonnegative additive cost on all finite raw executable paths, minimal raw-path cost induces a directed extended metric-like geometry invariant under cost-preserving operational recoding.

### Type domain `Dom(C)`

Operational objects for which the one-step execution relation determines the raw finite path set

```math
\Pi_{\rm raw}(\mathfrak O)
=
\{\pi:\pi\text{ is a finite typed path whose adjacent triples satisfy }\operatorname{Exec}\},
```

including empty paths, and `\kappa` semantically declares a cost function

```math
c:\Pi_{\rm raw}(\mathfrak O)\rightarrow[0,+\infty]
```

such that:

1. every empty path has cost `0`;
2. cost is determined by the frozen raw operational path;
3. for every typed raw-path concatenation `\pi_2\pi_1`,

   ```math
   c(\pi_2\pi_1)=c(\pi_1)+c(\pi_2).
   ```

Resource, budget, or horizon admissibility filters elsewhere in `\kappa` do not restrict `\Pi_{\rm raw}` for this formal baseline. They may define other operational objects or later candidates, but are not part of the distance domain here.

### Construction `C`

For `x,y` in the declared state carrier, define

```math
d_c(x,y)
=
\inf\{c(\pi):\pi\in\Pi_{\rm raw}(\mathfrak O),\ s(\pi)=x,\ t(\pi)=y\},
```

with `d_c(x,y)=+\infty` when no raw executable path exists from `x` to `y`.
...
No symmetrization, resource filtering, embedding, or coordinate chart is added.
...
The raw path set is fixed by `\operatorname{Exec}` and the cost function is frozen inside operational `\kappa`. No resource filter, norm, embedding, symmetrization, rescaling, kernel, coordinate, or weighting is chosen by the candidate.

### Output equivalence `\cong`

Isometry of directed extended-distance spaces under transported states and cost-preserving operational recoding.
...
**F3 inclusion:** resource/accounting semantics are the explicit role of F3, and the candidate uses the prospectively typed cost field while keeping its raw path domain fixed.  
**F5 inclusion:** exact recoding tests whether the raw-path cost geometry is representation invariant.
...
`CONDITIONAL_IF:` the distance requires an unfrozen resource filter, norm, embedding, symmetrization, rescaling, or path weighting not fixed by `\operatorname{Exec}` and the declared cost field.
...
`CONTROL_FAILURE_IF:` the raw path domain is filtered by budget/horizon contrary to this contract, cost additivity is violated despite the declared type, path-cost accumulation or infimum computation is implemented incorrectly, or F5 cost transport fails.

### Forbidden stronger interpretation

Survival does not establish a geometry of budget- or horizon-constrained reachability, Euclidean geometry, manifold structure, local differential geometry, symmetry, or a universal scalar of revisability. As a `FORMAL_BASELINE`, positive validation does not reject `H_0`.
...
## B7. `S-DYN-001` — Congruence of action-availability equivalence under action dynamics

**Layer:** Dynamical  
**Epistemic class:** `STRUCTURE_HYPOTHESIS`  
**Non-tautology argument:** Equality of the current action-availability signature `\Theta_A` fixes only which actions are currently available. It does not, by the construction or semantic domain alone, impose equality of the action-availability signatures of the corresponding successor states. The primary claim is therefore the contingent preservation of `\sim_A` under declared action dynamics; the candidate retains raw successor behavior and does not repair failures of congruence.

### Scientific claim

STRUCTURE-001 tests whether exact equality of current action availability is dynamically stable under the declared one-step actions: states with the same currently available actions must, after any shared available action, transition to states with the same currently available actions.

### Type domain `Dom(C)`

Operational objects for which:

1. the parent V1 one-step future-transformability object is well typed:

   ```math
   \mathcal T^\kappa(x)\subseteq\mathcal A\times X;
   ```
2. each action `a` defines a deterministic partial transition under the same one-step `\kappa` semantics:
   ...
   \delta_a(x)=x'
   \iff
   (a,x')\in\mathcal T^\kappa(x).
   ```

### Construction `C`

Freeze the action-availability signature as the action projection of the parent future-transformability object:

```math
\boxed{
\Theta_A(x)
=
\pi_{\mathcal A}(\mathcal T^\kappa(x))
=
\{a\in\mathcal A:\exists x'\in X,\ (a,x')\in\mathcal T^\kappa(x)\}.
}
```

Define exact action-availability equivalence:
...
1. the exact quotient `Q_A`;
2. the raw partial transitions `\{\delta_a\}`;
3. for each action and equivalence class, the unmodified representative-level successor classes needed to test congruence.

No representative is selected to force descent, and no failed class is repaired or removed.

### Claim `P`

For every `x\sim_A y` and every shared available action

```math
a\in\Theta_A(x)=\Theta_A(y),
```

the deterministic successors preserve action-availability equivalence:

```math
\boxed{
\delta_a(x)\sim_A\delta_a(y).
}
```

Definedness is not a separate structure-discovery claim: it follows immediately from equality of `\Theta_A` and the frozen relation between `\Theta_A` and the partial-action domains.

Equivalently, `\sim_A` is a congruence for the declared partial action dynamics exactly when the boxed successor condition holds for every equivalence class and shared available action.
...
The signature is fixed exactly as the action projection of `\mathcal T^\kappa`; the action transitions are the declared deterministic one-step dynamics under the same semantics. No alternate signature, representative selection, approximate threshold, action ordering, coordinate, repair rule, or tie-break is introduced.

### Output equivalence `\cong`

Isomorphism of the action-availability quotient plus raw-action test object under operational transport, preserving availability classes, action labels under transported action bijection, and successor-class relations.
...
**F4 exclusion:** this candidate uses the target-derived action projection `\Theta_A` to define its current-state equivalence and is not admissible as an F4 sufficient-state construction under the V1 no-target-leak rule.

### Failure contract

`CONDITIONAL_IF:` action availability requires an unfrozen projection, approximate matching rule, representative-selection rule, or action-matching convention beyond the transported action alphabet.

`REPRESENTATION_DEPENDENT_IF:` an operational twin fails to preserve the action-availability quotient and congruence verdict under transport.

`REFUTED_IF:` there exist `x\sim_A y` and `a\in\Theta_A(x)=\Theta_A(y)` such that

```math
\delta_a(x)\not\sim_A\delta_a(y).
```

`CONTROL_FAILURE_IF:` determinism is violated despite the declared type, `\Theta_A` is not computed as the exact action projection of `\mathcal T^\kappa`, or F5 action transport fails.

### Forbidden stronger interpretation

Survival would establish only that the frozen action-availability equivalence is dynamically stable at tested scope. It does not establish full future-transformational equivalence, dynamic controllability, Markov sufficiency, corrigibility, or that action-availability equivalence is the correct quotient for every downstream task.
